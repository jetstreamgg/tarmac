...
export function NetworkSelect({
  ...
  disabled?: boolean;
  /**
   * A custom trigger body, replacing the pill — Convert's full-width Network
   * row is the whole clickable surface. Callers supplying this own its layout,
   * chevron included.
   */
  ...
  const onValueChange = useCallback(
    (value: string) => {
      ...
      if (nextChainId === walletChainId) return;
      handleSwitchChain({
        chainId: nextChainId,
    ...
    [walletChainId, handleSwitchChain, chains, searchParams, nextIntent, navigate, setSearchParams]
  );
  ...
  if (isStatic) {
    ...
    return children ? (
      ...
      <span
        className={cn(pillClasses, 'hover:border-glassBorder cursor-default hover:bg-transparent')}
  ...
  return (
    <Select value={String(activeChainId)} onValueChange={onValueChange}>
      ...
      <SelectTrigger
        ...
        className={cn(pillClasses, 'group')}
      >
