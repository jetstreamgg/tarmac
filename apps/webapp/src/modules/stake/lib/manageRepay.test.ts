...
describe('calculateMaxRepayable — legacy Repay.tsx math, verbatim', () => {
  ...
  it('caps at the full debt when the balance covers it', () => {
    expect(
      calculateMaxRepayable({ debtValue: usds('30000'), dust: DUST, balance: usds('50000') })
    ).toEqual(usds('30000'));
  });
  ...
  it('uses the full balance when the remaining debt stays above dust', () => {
    ...
    expect(
      calculateMaxRepayable({ debtValue: usds('70000'), dust: DUST, balance: usds('30000') })
    ).toEqual(usds('30000'));
  });
  ...
  it('caps at debt−dust when a full-balance repay would strand a sub-dust remainder', () => {
    ...
    expect(
      calculateMaxRepayable({ debtValue: usds('40000'), dust: DUST, balance: usds('30000') })
    ).toEqual(usds('10000'));
  });
  ...
  it('returns 0 when even the dust-capped repay is out of reach', () => {
    ...
    expect(
      calculateMaxRepayable({ debtValue: usds('30000.1'), dust: DUST, balance: usds('29999') })
    ).toEqual(usds('0.1'));
  });
});
