...
/**
 * The e2e wallet config, for builds with VITE_USE_MOCK_WALLET. A factory rather
 * than a module-scope config: App calls it only in mock mode, so a production
 * build drops this module and its test addresses instead of creating a third
 * wagmi config on every page load.
 */
