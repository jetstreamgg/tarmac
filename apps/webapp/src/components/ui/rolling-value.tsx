...
export function RollingValue({
  ...
  instant?: boolean;
  /** Roll the figure in on mount too — for a value that appears beside an
   * existing one (the entry grids' before→after side once an amount is
   * typed), so it rises into place instead of popping. */
  enter?: boolean;
}) {
  ...
  return (
    ...
    >
      ...
      <motion.span
        ...
        initial={
          (state.gen > 0 || enter) && !prefersReducedMotion && !instant ? { y: IN_Y, opacity: 0 } : false
        }
