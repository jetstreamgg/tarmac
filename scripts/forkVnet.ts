...
const forkVnets = async (chainType: string[]) => {
  ...
  const updatedData = existingData.filter((item: any) => !chainsToFork.includes(item.NETWORK));

  // Add the newly forked chains
  chainsToFork.forEach((chain: string, index: number) => {
    const testnetData = testnetsData[index];
    const adminEndpoint = testnetData.rpcs.find((x: any) => x.name === 'Admin RPC');
