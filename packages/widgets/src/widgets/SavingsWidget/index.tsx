...
const SavingsWidgetWrapped = ({
  ...
  const { savingsApprove, savingsSupply, batchSavingsSupply, savingsWithdraw, batchUpgradeAndSupply } =
  ...
  const batchUpgradeAndSupplyDisabled =
    [TxStatus.INITIALIZED, TxStatus.LOADING].includes(txStatus) ||
    isSupplyBalanceError ||
  ...
  const errorOnClick = () => {
    return widgetState.action === SavingsAction.SUPPLY
      ? originToken.symbol === dai.symbol
        ? upgradeAndSupplyOnClick()
        : shouldUseBatch
          ? batchSupplyOnClick()
          : supplyOnClick()
